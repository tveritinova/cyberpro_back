...
class BackApiTestCase(unittest.TestCase):
    ...
    def test_api_can_post_team(self):
        ...
        self.assertEqual(data['team_id'], team['team_id'])
    ...
    def test_api_can_post_and_get_tournament(self):
        ...
        result = self.client().get('/tournaments/'+str(tournament_id))
        self.assertEqual(result.status_code, 200)
        data = ast.literal_eval(result.data)
    ...
    def test_team_in_tournament(self):
        ...
        result = self.client().post('/tournaments', data={'tournament_id': tournament_id,
                                                          'team_id': team_id})
        self.assertEqual(result.status_code, 201)

        result = self.client().get('/tournaments/'+str(tournament_id)+'/teams')
        self.assertEqual(result.status_code, 200)
        data = ast.literal_eval(result.data)
        self.assertIn(data['teams_id'], team_id)

        result = self.client().get('/teams/'+str(team_id)+'/tournaments')
        self.assertEqual(result.status_code, 200)
        data = ast.literal_eval(result.data)
